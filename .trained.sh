...
python ./generate.py --data data/penn --checkpoint PTB-512-layers-4-dncv0.0.5-cuda1-debug.pt --words 1000 --cuda 1 --outf generated.txt
...
for (( i = 1; i < 7; i++ )); do
  python main.py --nhid 512 --nlayers 2 --nhlayers 2 --emsize 512 --debug --cell_size $(( 512 / 2**$i )) --read_heads 4 --nr_cells $((2**$i*2)) --cell_size $(( 512 / 2**$i )) --batch_size 25 --data data/penn --dropouti 0.4 --dropouth 0.25 --seed 141 --model DNC --epoch 50 --cuda 0 --save PTB-512-layers-2x2-cells_size-$(( 512 / 2**$i ))-cuda0.pt --log-interval 20 > PTB-512-layers-2x2-cells_size-$(( 512 / 2**$i ))-debug-cuda0.log
done
